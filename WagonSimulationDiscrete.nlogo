...
;;; layout-wagon: background image
;;; near, close-to: search radius
;;; cone-of-vision, vision-distance: used in conjunction with in-cone for sight perception
;;; cone-of-influence, influence-distance: actionable distance
;;; total-passengers: passenger number
;;; max-passengers-in-terminal: some cap on number of people
;;; travel-time: length of travel time for passengers
;;; chance-of-fire: possibility of a fire occuring at every tick
;;; chance-of-rescue: possibility of fire being contained (not put out)
;;; spread-of-fire: diameter of fire
;;; safety-threshold: threshold in distance from fire for agents to feel safe
;;; open/close door: allow access to the train

;;; Define global variables
globals [fire-emergency had-fire x-fire y-fire]

;;; Define breed and agent variables
breed [fires fire]
...
to run-simulation
  ...
  ;;; new fire starting
  if (random 100 < chance-of-fire) and (had-fire != true) [
    set fire-emergency true
    user-message(" A fire has started in the building")
   ]

  ;;; continue fire
  if (fire-emergency = true) [
    ifelse (had-fire = true)
    [
      ask patches with [(pxcor = x-fire + random spread-of-fire) and (pycor = y-fire + random spread-of-fire)]
      [ ignite ]
    ]
    [
      set had-fire true
      set x-fire (random 160) - 80
      set y-fire (random 80) - 40
      ask patches with [(pxcor = x-fire) and (pycor = y-fire)]
      [ ignite ]
    ]
   ]

  ;;; fire contained
  if (random 100 < chance-of-rescue) and (fire-emergency = true) [
    user-message("The fire has been contained and will stop spreading!")
    set fire-emergency false]
...
;;; Note that this is actually a reporter that returns the list of state definitions

to-report state-def-of-passengers
  report (list

   state "enter station"
   # when "burning" do "dying-cries" goto "enter station"
   # when "fire-nearby" do "alert-fire" goto "platform-escape"
   ...
   state "waiting"
    # when "burning" do "dying-cries" goto "waiting"
    # when "fire-nearby" do "alert-fire" goto "platform-escape"
   ...
   state "passenger"
   # when "burning" do "dying-cries" goto "passenger"
   # when "fire-nearby" do "alert-fire" goto "get-off-from-fire"
   ...
   state "get-seated"
    # when "burning" do "dying-cries" goto "get-seated"
    # when "seat-clear" do "move-to target set color white" goto "seated"
    # otherwise do "nothing" goto "passenger"
   end-state

   state "seated"
    # when "burning" do "dying-cries" goto "seated"
    # when "fire-nearby" do "alert-fire" goto "get-off-from-fire"
    # after-n-ticks (random 100 + travel-time) do "set color brown" goto "get-off"
   ...
   state "get-off"
     # when "burning" do "dying-cries" goto "get-off"
     # when "fire-nearby" do "alert-fire" goto "get-off-from-fire"
     # on-success "goto-door-area" do "nothing" goto "exiting"
     # otherwise do "nothing" activate-machine "goto-door-area"
   end-state

   state "get-off-from-fire"
     # when "burning" do "dying-cries" goto "get-off-from-fire"
     # on-success "goto-door-area" do "nothing" goto "platform-escape"
     # otherwise do "nothing" activate-machine "goto-door-area"
   end-state

  state "exiting"
     # when "burning" do "dying-cries" goto "exiting"
     # when "fire-nearby" do "alert-fire" goto "platform-escape"
  ...
  state "platform-escape"
     # when "burning" do "dying-cries" goto "platform-escape"
     # on-success "walk-toward near 300" do "die" goto "platform-escape"
     # on-failure "walk-toward near 300" do "nothing" goto "get-off"
     # when "fire-nearby" do "select-escape-path" activate-machine "walk-toward near 300"
     # otherwise do "panic-run" goto "platform-escape" ;; increases the chaos factor upon continuous failures
  end-state
...
to-report burning
  report any? neighbors with [pcolor = orange]
end

to dying-cries
  user-message("I'm dead oh no")
  die
end

to alert-fire
   set color pink
   ;;user-message("I see a fire")
end

to-report fire-nearby
  report any? fire-in-sight
end

to-report fire-in-sight
  report fires in-cone vision-distance cone-of-vision
end

to select-escape-path
  let nearest-exit-dist [distance myself] of min-one-of patches with [pcolor = magenta][distance myself] ;; distance to nearest exit
  let nearest-fire-dist [distance myself] of min-one-of fires [distance myself] ;; distance to nearest fire

  let exit-1-dist [distancexy -3 30] of min-one-of fires [distancexy -4 30] ;; fire to exit at the top (hardcoded)
  let exit-2-dist [distancexy -37 -20] of min-one-of fires [distancexy -37 -20] ;; fire to exit at the bottom left (hardcoded)
  let exit-3-dist [distancexy 31 -20] of min-one-of fires [distancexy 31 -20] ;; fire to exit at the top (hardcoded)

  ;; runs to nearest exit based on instinct
  ifelse (nearest-exit-dist - nearest-fire-dist > safety-threshold) [ ;; ideally, safety-threshold=spread-of-fire but more cautious people might disagree
    set target min-one-of patches with [pcolor = magenta] [distance myself] ;; inaccurate measurement for good reason
  ]
  ;; stops and rely on deliberative thinking to determine where to run based on distance of the fire to exit
  [
    wait 1.0 ;; 'punishment' for slower thinking
    let exit-index position (max (list exit-1-dist exit-2-dist exit-3-dist)) (list exit-1-dist exit-2-dist exit-3-dist)
    ifelse (exit-index = 0)
      [set target patches with [(pxcor = -3) and (pycor = 30)] ]
      [ ifelse (exit-index = 1)
          [set target one-of patches with [(pxcor = -37) and (pycor = -20)] ]
          [set target one-of patches with [(pxcor = 31) and (pycor = -20)] ]
      ]
  ]
end
...
;;; Select a door with the fewer passengers.
...
to-report passenger-close
  report any? other passengers-on neighbors
end
...
;;; This is the list of free neighbors
